			...
			<div className="flex flex-col">
				<div className={`table-container ${containerClassName || ''}`.trim()}>
					<table>
						...
						<tbody>
							{loader && isLoading
								? Array.from({ length: loaderLength || 10 }).map((_, i) => (
												...
												{row.getVisibleCells().map((cell) => {
													return (
														<td
															onClick={
																onRowClick &&
																!['actions', 'checked'].includes(cell.column.id)
																	? (e) => {
								...
								{table.getFooterGroups().map((footerGroup, index) => {
									return (
										<React.Fragment key={footerGroup.id + `-${index}`}>
											<tr className="table-footer-row-horizonal">
												...
												<>
													{Array.from({ length: footerRowCount - 1 }).map(
														(_, i) => {
															...
															return (
																<tr
																	className="table-footer-row-horizonal"
																	key={key}
																>
